...
#[derive(Debug)]
pub enum BgpVrfMsg {
    /// Atomically committed per-neighbour, per-family prefix-set binding.
    /// The VRF task owns the live Peer and policy subscription, so the
    /// global config callback forwards only the desired name (or None).
    PrefixSetConfig {
        address: IpAddr,
        afi_safi: AfiSafi,
        direction: InOut,
    ...
    /// End of BGP's side of one config transaction.  FIFO ordering on the
    /// per-VRF inbox places this after every `PrefixSetConfig` generated by
    /// that transaction; the receiver can now apply policy-watch updates
    /// buffered since `PolicyRx::PrefixSetCommitStart`.
    PrefixSetCommitEnd { generation: u64 },
    ...
    /// (`ORIGINATED_PEER` / remote 0) from the VRF Loc-RIB and emit
